  ...
  return (
    <div className="flex flex-col min-h-full">
      <div className="flex-1 max-w-full px-6 py-6">
        ...
        {/* Header */}
        <div className="page-enter flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6">
          <div>
          ...
          <div className="flex items-center gap-3">
            <div className="relative hidden sm:block">
              ...
              <input
                ...
                className="bg-[#1a1a1a] border border-[#2a2a2a] rounded-lg pl-9 pr-4 py-2 text-white text-sm placeholder-gray-600
                  outline-none focus:border-[#D4F547] transition-colors w-56"
              />
            </div>
            <button className="p-2 rounded-lg bg-[#1a1a1a] border border-[#2a2a2a] text-gray-400 hover:text-white transition-colors duration-200">
              <Bell size={16} />
            </button>
        ...
        <div className="page-enter-delay-1 grid grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
          {loading ? (
            [1,2,3,4].map((i) => <SkeletonStatsCard key={i} />)
          ) : (
            <>
              <StatsCard label="Total Reports This Month" value={stats?.totalReports ?? 0} trend={12} icon={FileText} />
              <StatsCard label="Active Cases"             value={stats?.activeCases ?? 0} trend={-4} icon={CheckSquare} />
              <StatsCard label="Resolution Rate"          value={`${stats?.resolutionRate ?? 0}%`} trend={3} icon={Percent} />
              <StatsCard label="Pending Verifications"    value={stats?.pendingVerifications ?? 0} trend={-5} icon={Clock} />
            </>
          )}
        </div>

        {/* Main grid */}
        <div className="page-enter-delay-2 grid grid-cols-1 xl:grid-cols-3 gap-5">
