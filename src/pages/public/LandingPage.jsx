...
function LandingNavbar() {
  return (
    <nav className="bg-[#0a0a0a]/95 backdrop-blur-md border-b border-[#1a1a1a] sticky top-0 z-40 transition-shadow duration-300">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 h-14 flex items-center justify-between">
        <div className="flex items-center gap-2.5 group">
          <div className="w-8 h-8 bg-[#D4F547] rounded-lg flex items-center justify-center transition-all duration-250 group-hover:scale-105 group-hover:shadow-[0_0_14px_rgba(212,245,71,0.3)]">
            <Search size={16} className="text-black" />
          </div>
          <span className="text-white font-bold text-sm tracking-tight">lost-found</span>
        </div>
        <div className="hidden md:flex items-center gap-6">
          <a href="#features" className="text-gray-400 text-sm hover:text-white transition-colors duration-200">Features</a>
          <a href="#pipeline" className="text-gray-400 text-sm hover:text-white transition-colors duration-200">How It Works</a>
          <Link to="/browse" className="text-gray-400 text-sm hover:text-white transition-colors duration-200">Browse</Link>
        </div>
        <div className="flex items-center gap-3">
          <Link to="/login" className="text-gray-300 text-sm hover:text-white transition-colors duration-200 px-2 py-1 rounded">
            Login
          </Link>
          <Link
            to="/signup"
    ...
    <div className="min-h-screen bg-[#0a0a0a] flex flex-col">
      ...
      <section className="flex-1 max-w-7xl mx-auto px-4 sm:px-6 py-16 md:py-24 flex flex-col md:flex-row items-center gap-12">
        ...
        <div className="flex-1">
          <div className="page-enter inline-flex items-center bg-[#D4F547]/10 border border-[#D4F547]/20 rounded-full px-4 py-1.5 mb-6
            transition-all duration-300 hover:bg-[#D4F547]/15 hover:border-[#D4F547]/30 cursor-default">
            <span className="w-1.5 h-1.5 rounded-full bg-[#D4F547] mr-2 animate-pulse" />
            <span className="text-[#D4F547] text-xs font-semibold tracking-wide">Secure Campus Infrastructure</span>
          </div>
          <h1 className="page-enter-delay-1 text-4xl md:text-5xl lg:text-6xl font-bold text-white leading-tight mb-6">
            Never Lose<br />Track Again
          </h1>
          <p className="page-enter-delay-2 text-gray-400 text-lg leading-relaxed mb-8 max-w-lg">
          ...
          <div className="page-enter-delay-3 flex flex-wrap gap-3">
            <Link
              to="/report-lost"
              className="border border-white/20 text-white px-6 py-3 rounded-lg text-sm font-medium
        ...
        <div className="page-enter-delay-2 flex-1 max-w-sm md:max-w-none">
          <div className="relative bg-[#D4F547] rounded-2xl p-8 aspect-square max-w-xs mx-auto md:max-w-sm
            transition-all duration-500 hover:shadow-[0_20px_60px_rgba(212,245,71,0.18)] hover:-translate-y-1">
            ...
            {features.map(({ icon: Icon, title, desc }, i) => (
              <div
                ...
                className={`reveal reveal-delay-${i + 1}
                  ...
                  hover:border-[#3a3a3a] group`}
              >
                <div className="w-10 h-10 bg-[#D4F547]/10 rounded-lg flex items-center justify-center mb-4
                  transition-all duration-250 group-hover:bg-[#D4F547]/18 group-hover:scale-105">
                  <Icon size={20} className="text-[#D4F547]" />
                </div>
                <h3 className="text-white font-semibold mb-2 transition-colors duration-200 group-hover:text-white/90">{title}</h3>
                <p className="text-gray-500 text-sm leading-relaxed transition-colors duration-200 group-hover:text-gray-400">{desc}</p>
              </div>
            ))}
          </div>
        </div>
      ...
      <section id="pipeline" className="py-16 md:py-24">
        <div className="max-w-7xl mx-auto px-4 sm:px-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            {pipelineSteps.map(({ num, title, desc }, i) => (
              <div
                key={num}
                className={`reveal reveal-delay-${i + 1} group`}
              >
                <div className="flex items-center gap-4 mb-4">
                  <span className="text-4xl font-black text-[#D4F547] leading-none transition-all duration-300 group-hover:text-[#c8ee38] group-hover:scale-105">
                    {num}
                  </span>
                  <div className="flex-1 h-px bg-[#2a2a2a] transition-colors duration-300 group-hover:bg-[#3a3a3a]" />
                </div>
                <h3 className="text-white font-semibold text-lg mb-2 transition-colors duration-200 group-hover:text-white/90">{title}</h3>
                <p className="text-gray-500 text-sm leading-relaxed transition-colors duration-200 group-hover:text-gray-400">{desc}</p>
              </div>
            ))}
          </div>
          ...
          <div className="reveal mt-16 text-center">
            ...
            <div className="flex flex-wrap gap-3 justify-center">
              <Link
                to="/signup"
                className="bg-[#D4F547] text-black font-semibold px-8 py-3 rounded-lg text-sm
                  btn-lime-glow btn-ripple"
              >
                Create Free Account
              </Link>
              <Link
                to="/browse"
                className="border border-[#2a2a2a] text-white px-8 py-3 rounded-lg text-sm
                  hover:bg-[#1a1a1a] hover:border-[#3a3a3a]
                  transition-all duration-200 btn-ripple"
              >
                Browse Items
