        ...
        {loading ? (
          ...
          <div className="page-enter">
            <EmptyState
              ...
              action={
                <Link to="/browse">
                  <button className="bg-[#D4F547] text-black font-semibold px-5 py-2 rounded-lg text-sm hover:bg-[#c2e040] transition-colors">
                    Browse Items
                  </button>
                </Link>
              }
          ...
          <div className="bg-[#1a1a1a] border border-[#2a2a2a] rounded-xl divide-y divide-[#1f1f1f] overflow-hidden">
            {claims.map((claim, i) => (
              <div
                key={claim.id}
                className="px-5 py-4 flex items-center justify-between gap-4
                  hover:bg-[#1f1f1f] transition-all duration-200
                  opacity-0 animate-[fade-up_0.35s_ease-out_forwards]"
                ...
                <div className="flex items-center gap-3">
                  ...
                  {claim.status === 'pending' && (
                    <Link to={`/claims/${claim.id}/verify`}>
                      <button className="flex items-center gap-1 text-[#D4F547] text-xs hover:underline group">
                        Continue
